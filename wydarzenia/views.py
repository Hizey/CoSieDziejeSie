...
def loginPage(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            messages.error(request, '')    #https://docs.djangoproject.com/en/4.1/ref/contrib/messages/

            user = authenticate(request, username=username, password=password)

            if user is not None:
                login(request, user)
                return redirect('home')         #add to database
            else:
...
def registerPage(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request, 'error rejestracji')
...
def updateRoom(request, pk):
    ...
    if request.user != room.host:
        return HttpResponse('Nie możesz edytować czyichś wydarzeń')
...
def deleteRoom(request, pk):
    ...
    if request.user != room.host:
        return HttpResponse('Nie możesz usunąć czyjegoś wydarzenia')
